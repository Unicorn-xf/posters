...
module.exports = {
	formatDateTime(date) {
		// 处理时间少8小时问题,以后unicloud云函数node js 可以配置环境变量Key:TZ,Value:Asia/Shanghai 可以去掉
	...
	getWeekStartDate() {
		...
		return standardFormatDate(weekStartDate);
	},
	...
	getWeekEndDate() {
		...
		return standardFormatDate(weekEndDate);
	},
	//获得本月的开始日期
	getMonthStartDate() {
		var now = new Date(); //当前日期
		...
		return standardFormatDate(monthStartDate);
	},
	...
	getMonthEndDate() {
		...
		return standardFormatDate(monthEndDate);
	},
	//获得本季度的开始日期
	getQuarterStartDate() {
		var now = new Date(); //当前日期
		...
		return standardFormatDate(quarterStartDate);
	},
	...
	getQuarterEndDate() {
		...
		var quarterStartDate = new Date(nowYear, quarterEndMonth, getMonthDays(quarterEndMonth,nowYear));
		return standardFormatDate(quarterStartDate);
	},
	//获得本年的开始日期
	getYearStartDate() {
		...
		return standardFormatDate(yearStartDate);
	},
	...
	getYearEndDate() {
		...
		var nowYear = now.getYear(); //当前年
		nowYear += (nowYear < 2000) ? 1900 : 0;
		var yearEndDate = new Date(nowYear, 11, getMonthDays(11, nowYear));
		return standardFormatDate(yearEndDate);
	},
	getWeekDay(data) {
		let dayNames = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"]
		return dayNames[data.getDay()]
	},
	// app请求成功返回
	serverSuccess(data, retmsg) {
		return {
			"retcode": "0000",
	...
	async getSysArgument(data) {
		// // console.log("data----"+JSON.stringify(data))
		let argValue
		try {
			argValue = await db.collection('sys_argument')
				.where({
			...
			return argValue.data.length === 0 ? null : argValue.data[0].arg_value
		} catch (err) {
